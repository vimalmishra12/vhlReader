...
module.exports = {
    ...
    noteListItems: selectorFile.vhlNotes.noteListItems,
    spanTag: selectorFile.vhlNotes.spanTag,
    deletebuttontag: selectorFile.vhlNotes.deletebuttontag,
    deleteTid: selectorFile.vhlNotes.deleteTid,
    ...
    getData_vhlNotes: async function () {
        ...
        obj = {
            ...
            noteListItems: ((await action.getElementCount(this.noteListItems)) > 0) ? await action.getText(this.noteListItems) : null,
            spanTag: ((await action.getElementCount(this.spanTag)) > 0) ? await action.getText(this.spanTag) : null,
            deletebuttontag: ((await action.getElementCount(this.deletebuttontag)) > 0) ? await action.getText(this.deletebuttontag) : null,
            deleteTid: ((await action.getElementCount(this.deleteTid)) > 0) ? await action.getText(this.deleteTid) : null,
        }
        return obj;
    },
    ...
    // await logger.logInto(await stackTrace.get());
    // console.log("noteslist element reference:", this.noteListItems);
    // console.log("deleteNoteSvg element reference:", this.deleteNoteSvg);
    // var res;
    ...
    click_deleteNoteSvg: async function (noteTest) {

        await logger.logInto(await stackTrace.get());

        const noteText = noteTest.text;
        const targetLi = await action.findNoteListItemByText(noteText, this.noteListItems, this.spanTag);

        if (!targetLi) {
            await logger.logInto(await stackTrace.get(), "No matching note found", 'error');
            return null;
        }

        // const deleteButton = await targetLi.$("div > div > button[data-tid]");
        const deleteButton = await action.childElements(targetLi, this.deletebuttontag);
        const deleteButtonFirst = deleteButton[0];
        if (deleteButtonFirst) {
            const dataTid = await action.getAttributeByElement(deleteButtonFirst, this.deleteTid);
            const res = await action.click(deleteButtonFirst);
            if (res === true) {
                await logger.logInto(await stackTrace.get(), `deleteNoteSvg (${dataTid}) is clicked`);
            } else {
                await logger.logInto(await stackTrace.get(), res + " deleteNoteSvg is NOT clicked", 'error');
            }
            return res;
        ...
        await logger.logInto(await stackTrace.get(), "Delete button not found", 'error');
        return null;
    },
