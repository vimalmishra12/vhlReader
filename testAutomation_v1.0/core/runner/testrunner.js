...
class specRunner {
  ...
  async runMochaSuite() {
    try {
      ...
      var that = this;
      var visualTotalTc = 0;
      execJsonData = await jsonParserUtil.jsonParser(
        path.resolve(rootDir, testExecDir, argv.testEnv, this.testExecFile)
      );
      Object.keys(execJsonData).forEach(async (suiteIndex, count) => {
        Arr[count] = {
          title: suiteIndex + " " + execJsonData[suiteIndex].Name,
          duration: "-",
          start: "-",
          end: "-",
          tests: [],
        ...
        describe(suiteIndex + " - " + execJsonData[suiteIndex].Name, () => {
          ...
          before(async function () {
            ...
            console.log(
              "====== Starting Test " +
                suiteIndex +
                ": " +
                execJsonData[suiteIndex].Name +
                " ======"
            );

            if (argv.visual == "applitools")
              await that.visualTest.openApplitoolsEyes(
                suiteIndex,
                execJsonData[suiteIndex].Name
              );

            global.logger = await require(path.join(
              rootDir,
              "/core/utils/loggerFunction.js"
            ));
            logger.logInto(
              stackTrace.get(),
              "Starting Test Suite:" +
            ...
            for (const key of beforeGroup) {
              console.log(
                " Before      : Start " +
                  execJsonData[suiteIndex].Before[key].id
              );
              await logger.logInto(
                stackTrace.get(),
                "BEFORE HOOK:" +
                  JSON.stringify(execJsonData[suiteIndex].Before[key])
              );
              let prop = that.getTCPropertiesFromTCRepo(
                execJsonData[suiteIndex].TestCaseRepo,
                execJsonData[suiteIndex].Before[key].testFile,
                execJsonData[suiteIndex].Before[key].id
              );
              global.selectorDir = prop.selectorDir;
              if (prop.skipTC) console.log("               Skipped ");
              else
                await that.jsonHookObjParser(
                  execJsonData[suiteIndex].Before[key]
                );
              console.log(
                " Before      :   End " +
                  execJsonData[suiteIndex].Before[key].id
          ...
          beforeEach(async function () {
            let beforeEachGroup = Object.keys(
              execJsonData[suiteIndex].BeforeEach
            );
            for (const key of beforeEachGroup) {
              console.log(
                " Before each : Start " +
                  execJsonData[suiteIndex].BeforeEach[key].id
              );
              await logger.logInto(
                stackTrace.get(),
                "BEFORE EACH HOOK: " +
                  JSON.stringify(execJsonData[suiteIndex].BeforeEach[key])
              );
              let prop = that.getTCPropertiesFromTCRepo(
                execJsonData[suiteIndex].TestCaseRepo,
                execJsonData[suiteIndex].BeforeEach[key].testFile,
                execJsonData[suiteIndex].BeforeEach[key].id
              );
              global.selectorDir = prop.selectorDir;
              if (prop.skipTC) console.log("               Skipped ");
                ...
                await that.jsonHookObjParser(
                  execJsonData[suiteIndex].BeforeEach[key]
                );
              console.log(
                " Before each :   End " +
                  execJsonData[suiteIndex].BeforeEach[key].id
          ...
          Object.keys(execJsonData[suiteIndex].Test).forEach(function (
            testIndex
          ) {
            ...
            tcProp = that.getTCPropertiesFromTCRepo(
              execJsonData[suiteIndex].TestCaseRepo,
              execJsonData[suiteIndex].Test[testIndex].testFile,
              execJsonData[suiteIndex].Test[testIndex].id,
              execJsonData[suiteIndex].Test[testIndex].description
            );
            if (argv.visual == "novus") {
              let visual = execJsonData[suiteIndex].Test[testIndex].visualTest;
              if (visual == undefined) visual = tcProp.visualTag;
            ...
            var arrIndex = 0;
            Object.keys(
              execJsonData[suiteIndex].Test[testIndex].testData
            ).forEach(async (tdIndex) => {
              //condition for checking if test data is a string or an array
              if (
                typeof execJsonData[suiteIndex].Test[testIndex].testData[
                  tdIndex
                ].jsonPath == "string"
              ) {
                //condition for accepting login data file via command
                if (
                  execJsonData[suiteIndex].Test[testIndex].testFile.includes(
                    "login.test.js"
                  ) &&
                  argv.loginDataFile
                )
                  ...
                  testDataArr[suiteIndex][testIndex][arrIndex] =
                    jsonParserUtil.getJsonNode(
                      jsonParserUtil.jsonParser(
                        path.resolve(rootDir, argv.loginDataFile)
                      ),
                      execJsonData[suiteIndex].Test[testIndex].testData[tdIndex]
                        .jsonPath
                    );
            ...
            if (testDataArr[suiteIndex][testIndex].length == 1) {
              testDataArr[suiteIndex][testIndex] =
                testDataArr[suiteIndex][testIndex][0];
            }
            ...
            it(getName, async function () {
              ...
              if (tcProp.skipTC) {
                ...
                if (
                  ...
                  Arr[count] = await that.visualTest.generateScreenshotsAndLogs(
                    execJsonData[suiteIndex].Test[testIndex],
                    suiteIndex,
                    testIndex,
                    Arr,
                    count
                  ...
                  await that.visualTest.generateScreenshotsApplitools(
                    execJsonData[suiteIndex].Test[testIndex]
                  );
              }
              console.log(
          ...
          afterEach(async function () {
            let afterEachGroup = Object.keys(
              execJsonData[suiteIndex].AfterEach
            );
            for (const key of afterEachGroup) {
              console.log(
                " After each  : Start " +
                  execJsonData[suiteIndex].AfterEach[key].id
              );
              logger.logInto(
                stackTrace.get(),
                "AFTER EACH HOOK: " +
                  JSON.stringify(execJsonData[suiteIndex].AfterEach[key])
              );
              let prop = that.getTCPropertiesFromTCRepo(
                execJsonData[suiteIndex].TestCaseRepo,
                execJsonData[suiteIndex].AfterEach[key].testFile,
                execJsonData[suiteIndex].AfterEach[key].id
              );
              global.selectorDir = prop.selectorDir;
              if (prop.skipTC) console.log("               Skipped ");
                ...
                await that.jsonHookObjParser(
                  execJsonData[suiteIndex].AfterEach[key]
                );
              console.log(
                " After each  :   End " +
                  execJsonData[suiteIndex].AfterEach[key].id
          ...
          after(async function () {
            ...
            for (const key of afterGroup) {
              console.log(
                " After       : Start " + execJsonData[suiteIndex].After[key].id
              );
              logger.logInto(
                stackTrace.get(),
                "AFTER HOOK: " +
                  JSON.stringify(execJsonData[suiteIndex].After[key])
              );
              let prop = that.getTCPropertiesFromTCRepo(
                execJsonData[suiteIndex].TestCaseRepo,
                execJsonData[suiteIndex].After[key].testFile,
                execJsonData[suiteIndex].After[key].id
              );
              global.selectorDir = prop.selectorDir;
              if (prop.skipTC) console.log("               Skipped ");
              else
                await that.jsonHookObjParser(
                  execJsonData[suiteIndex].After[key]
                );
              console.log(
                " After       :   End " + execJsonData[suiteIndex].After[key].id
              );
            }
            if (argv.visual == "applitools")
      ...
      after(() => {
        ...
        if (argv.visual)
          that.visualTest.setVisualReportData(
            this.testExecFile,
            visualTotalTc,
            this.startTime,
            endTime,
  ...
  async identifyTest(testFile, testFunction, testdata) {
    ...
    if (prevtestFile != testFile) {
      ...
      try {
        ...
        logger.logInto(
          stackTrace.get(),
          "ERROR!! " + testFilepath + " NOT found." + e
        );
  ...
  async jsonHookObjParser(obj) {
    ...
    for (const dataIndex of jsonHookObjParserGroup) {
      ...
      if (typeof obj.testData[dataIndex].jsonPath == "string") {
        if (obj.testFile.includes("login.test.js") && argv.loginDataFile) {
          ...
          hookFuncData[arrIndex] = await jsonParserUtil.getJsonNode(
            jsonParserUtil.jsonParser(argv.loginDataFile),
            obj.testData[dataIndex].jsonPath
          );
        } else {
          ...
          hookFuncData[arrIndex] = await jsonParserUtil.getJsonNode(
            jsonParserUtil.jsonParser(obj.testData[dataIndex].dataFile),
            obj.testData[dataIndex].jsonPath
          );
        }
        arrIndex++;
      } else {
        let jsonGroup = Object.keys(
          Object.keys(obj.testData[dataIndex].jsonPath)
        );
    ...
    if (hookFuncData.length == 1) {
      hookFuncData = hookFuncData[0];
    }
  ...
  getTCPropertiesFromTCRepo(tcRepo, moduleTestFile, tcId, tcDescription) {
    ...
    tcRepo.forEach(async (repoFile, index) => {
      repository[index] = jsonParserUtil.jsonParser(
        path.resolve(rootDir, repoFile)
      );
    });
    for (repoKey = 0; repoKey < repository.length; repoKey++) {
      for (
        moduleKey = 0;
        moduleKey < repository[repoKey].modules.length;
        moduleKey++
      ) {
        ...
        if (repository[repoKey].modules[moduleKey].testFile == moduleTestFile) {
          for (
            tcKey = 0;
            tcKey < repository[repoKey].modules[moduleKey].testcase.length;
            tcKey++
          ) {
            ...
            if (testObj.id == tcId) {
              ...
              obj.tcName =
                testObj.id +
                " " +
                (tcDescription == "" ? testObj.description : tcDescription) +
                " - (" +
                testObj.tags +
                ")";
              obj.visualTag = testObj.visualTest;
              obj.selectorDir = path.resolve(
                rootDir,
                repository[repoKey].selectorFile
              );
              obj.skipTC = false;
              if (
                moduleOff != undefined &&
                moduleOff[repository[repoKey].modules[moduleKey].moduleId]
              )
                obj.skipTC = true;
              break;
            }
