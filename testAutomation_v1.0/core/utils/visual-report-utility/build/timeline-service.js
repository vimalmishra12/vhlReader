...
// const config = require('./wdio.conf.js').config.reporters;
//console.log(config)
...
class TimelineService {
  ...
  onPrepare() {
    ...
    try {
      // mkdir recursively
      const initDir = path_1.isAbsolute(this.resolvedOutputDir)
        ? path_1.sep
        : '';
      ...
      this.watcher = fs_1.watch(
        this.resolvedOutputDir,
        (eventType, filename) => {
          if (filename.includes('visualReport')) {
            fs_1.appendFileSync(this.changeLogFile, `${filename}\n`);
          }
        }
      );
    } catch (error) {
      console.log('on onPrepare');
      console.log(error);
    }
  }
    ...
    return __awaiter(this, void 0, void 0, function* () {
      if (this.reporterOptions.images && this.reporterOptions.images.resize) {
        ...
        const promises = screenshots.map((filePath) =>
          utils_1.waitForFileExistsAndResize(filePath, quality, reductionRatio)
        );
  ...
  generateTestResults(results) {
    ...
    //const totalDuration = Math.abs(new Date(results[0].end) - new Date(results[0].start));
  ...
  onComplete() {
    const folderAndChangeLogFileExists =
      fs_1.existsSync(this.resolvedOutputDir) &&
      fs_1.existsSync(this.changeLogFile);
    // close watcher
    this.watcher.close();
    if (folderAndChangeLogFileExists) {
      const runnerLogFiles = fs_1
        .readFileSync(this.changeLogFile, 'utf-8')
        .split('\n')
        .filter((line) => line !== '');
      const results = [];
      ...
      const screenshots = utils_1.deepSearch(
        'screenshots',
        combinedTestResults
      );
